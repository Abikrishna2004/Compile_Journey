...
import { ChevronDown, ChevronUp, Code2, Brain, Rocket, User } from 'lucide-react';
                ...
                <div className="glass-effect rounded-2xl border border-white/10 p-8 lg:p-10 w-full shadow-2xl relative overflow-hidden reveal">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
                        ...
                        <div className="lg:col-span-3 flex flex-col items-center lg:items-center text-center border-b lg:border-b-0 lg:border-r border-white/10 pb-8 lg:pb-0 lg:pr-8">
                            ...
                            <h3 className="text-3xl font-space font-bold text-white tracking-tight mb-1">Abikrishna</h3>
                            <p className="text-sky-400 font-inter font-medium text-sm w-full uppercase tracking-wider">Founder & Developer</p>
                        </div>

                        {/* Column 2: Content (6 cols) */}
                        <div className="lg:col-span-6 space-y-6">
                            <div>
                                <h2 className="text-xs font-space font-bold tracking-[0.2em] text-sky-500 uppercase mb-3">Behind the Code</h2>
                                <h3 className="text-3xl md:text-5xl font-space font-bold text-white leading-tight">
                                    The Story of <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-purple-400 to-purple-500">Compile Journey</span>
                                </h3>
                            </div>

                            <div className="text-slate-300 font-inter text-base md:text-lg leading-relaxed space-y-4 font-light tracking-wide">
                                <p>
                                    My journey began with a simple fascination: how can lines of text on a screen change the world? That curiosity led me down the rabbit hole of coding, AI, and problem-solving.
                                </p>

                                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 md:max-h-[500px] md:opacity-100'}`}>
                                    <p className="mb-4">
                                        Through dedication and countless hours of experimentation, I've honed my skills in <strong className="text-white font-semibold">Python, FastAPI, and LLMs</strong>. I built <em>Compile Journey</em> not just as a portfolio, but as a platform to share knowledge, showcase innovation, and demonstrate the power of modern web technologies.
                                    </p>
                                    <p>
                                        Today, I focus on bridging the gap between complex data science concepts and intuitive user experiences, creating tools like <strong className="text-white font-semibold">Avira</strong> that make AI accessible to everyone.
                                    </p>
                                </div>
                        ...
                        <div className="lg:col-span-3 flex flex-row lg:flex-col gap-4 flex-wrap lg:border-l border-white/10 lg:pl-8">
                            {highlights.map((item, index) => (
                                <div key={index} className="flex-1 lg:flex-none flex items-center gap-3 bg-white/5 rounded-xl p-4 border border-white/5 hover:bg-white/10 hover:border-sky-500/30 hover:translate-x-1 transition-all duration-300 group">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-xs text-slate-400 uppercase tracking-wider">Skill</span>
                                        <span className="text-sm font-bold text-white group-hover:text-sky-200 transition-colors">{item.label}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
