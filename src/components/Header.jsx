...
const Header = ({ isMenuOpen, setIsMenuOpen }) => {
    ...
    return (
        <>
            {/* Mobile Menu Backdrop - Optional for push menu, but good for closing */}
            <div
                className={`lg:hidden fixed inset-0 bg-black/50 backdrop-blur-[2px] z-[90] transition-opacity duration-300 ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}
                onClick={toggleMenu}
            ></div>
            ...
            <header id="header" className={`glass-effect fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b border-white/5 backdrop-blur-xl ${isMenuOpen ? 'translate-x-64' : ''}`}>
                ...
                <div className="container mx-auto px-4 sm:px-6 py-4 flex justify-between items-center relative">
                    ...
                    <button onClick={toggleMenu} className="lg:hidden interactive-element text-white hover:text-sky-400 transition-colors relative z-[110]">
